...
export class PurrSynth extends BaseSonifier {
  ...
  private breathLFOGain: GainNode | null = null;
  
  private exciter: OscillatorNode | null = null;
  private exciterLPF: BiquadFilterNode | null = null;
  
  private f1Res: BiquadFilterNode | null = null;
  private f2Res: BiquadFilterNode | null = null;
  private f1Gain: GainNode | null = null;
  private f2Gain: GainNode | null = null;

  private rumbleOsc: OscillatorNode | null = null;
  private rumbleLPF: BiquadFilterNode | null = null;
  private rumbleGain: GainNode | null = null;

  private jitterOsc: OscillatorNode | null = null;
  private jitterGain: GainNode | null = null;
  ...
  private createPulseWave(): PeriodicWave {
    const n = 64;
  ...
  private updateNodes(immediate: boolean = false): void {
    if (!this.isInitialized && !immediate) return;
    
    // Ensure nodes exist before update
    if (!this.exciter || !this.exciterLPF || !this.f1Res || !this.f2Res || !this.f1Gain || !this.f2Gain || !this.rumbleOsc || !this.rumbleLPF || !this.rumbleGain || !this.jitterOsc || !this.jitterGain || !this.breathLFO || !this.breathLFOGain) return;
    ...
    const mapParam = (id: string) => {
      const desc = this.descriptor.parameters[id] as any;
      const val = this.params[id] as number;
    ...
    const setFreq = (param: AudioParam, val: number) => {
      if (immediate) param.setValueAtTime(val, now);
      else param.setTargetAtTime(val, now, ramp);
    };

    const setGain = (param: AudioParam, val: number) => {
      if (immediate) param.setValueAtTime(val, now);
      else param.setTargetAtTime(val, now, ramp);
    };
    ...
    setGain(this.jitterGain.gain, purrRateHz * (this.params.jitterDepth as number) * 0.5);
